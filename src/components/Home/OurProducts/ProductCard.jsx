...
const ProductCard = ({product}) => {
  return (
    <div className="product-card flex flex-col items-center justify-center border border-black border-opacity-20 rounded-3xl py-6">
      ...
      <h3 className="title text-[1C121F] text-lg font-semibold font-Poppins">
        {product.title}
      </h3>
      <p className="desc text-xs text-[#1C121F] font-normal font-Poppins mt-2">
      {product.description}
      </p>
      <div className="usd-add-to-cart flex flex-row items-center justify-center space-x-9 mt-3.5">
        <span className="text-[#1C121F] text-lg font-semibold">${product.price}</span>
        <button className="bg-primaryColor text-white px-10 py-3.5 rounded-[50px]">
